(function (root, factory) {
  ...
  var loadDependence = function (fncallback) {
    ...
    if (!window.wwload.bubble) {
      window.wwload.bubble = "wait";
      // requirejs.config({
      //   paths: {
      //     "插件名": "插件路径" // 示例: libs/qrcodejs/qrcode
      //   }
      // });
      require(["css!libs/bubble/css/Bubble", "libs/bubble/js/Bubble"], function () {
        window.wwload.bubble = true;
        replace();
        fncallback();
      });
    } else if (window.wwload.bubble === "wait") {
      setTimeout(function () {
   ...
   */
  function bubble($ele) {
    var fontsize = eval($ele.attr("data-fontsize") || "16");
    var radius = eval($ele.attr("data-radius") || "140");
    var mspeed = $ele.attr("data-mspeed") || "slow";
    var ispeed = $ele.attr("data-ispeed") || "slow";
    var direction = eval($ele.attr("data-direction") || "135");
    var keep = eval($ele.attr("data-keep") || "false");
    tagcloud({
      selector: ".tagcloud",  //元素选择器
      fontsize: fontsize,       //基本字体大小, 单位px
      radius: radius,         //滚动半径, 单位px 页面宽度和高度的五分之一
      mspeed: mspeed,   //滚动最大速度, 取值: slow, normal(默认), fast
      ispeed: ispeed,   //滚动初速度, 取值: slow, normal(默认), fast
      direction: direction,     //初始滚动方向, 取值角度(顺时针360): 0对应top, 90对应left, 135对应right-bottom(默认)...
      keep: keep          //鼠标移出组件后是否继续随鼠标滚动, 取值: false, true(默认) 对应 减速至初速度滚动, 随鼠标滚动
    });
  }
  ...
  function processElement($ele) {
    ...
    setTimeout(function () {
      $.wwclass.helper.updateProp($ele, "data-x-inited", true);
      $.wwclass.helper.anijsTrigger($ele, "bubblesuccess");
